...
node {
    ...
    stage('Email Report') {
        sh "echo 'Please find the report attached.'| mail -s 'Shared Reporting Output' -a '${WORKSPACE}/report.csv' -r 'noreply@reform.hmcts.net (Shared Reporting)' Alliu.Balogun@HMCTS.NET James.Johnson@HMCTS.NET cftprogrammepmo@hmcts.gsi.gov.uk"       
    }
    stage('Cleanup') {
